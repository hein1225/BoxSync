...
> - **Docker 部署**：**强制要求 Redis 连接**，无 Redis 时服务无法启动，确保数据持久化
...
| 数据写入 | `POST` | `/api/sync/write` | 单条数据上传 |
| 数据读取 | `GET` | `/api/sync/read` | 单条数据下载 |
| 批量同步 | `POST` | `/api/sync/batch` | 多条数据批量上传 |
| 获取变更 | `GET` | `/api/sync/changes` | 增量同步，获取指定时间后的变更 |
| 删除数据 | `DELETE` | `/api/sync/delete` | 删除指定数据 |
| 应用列表 | `GET` | `/api/sync/apps` | 获取用户的应用分区列表 |
| 创建应用 | `POST` | `/api/sync/apps` | 创建新的应用分区 |
...
#### 认证方式
- 所有同步接口（除登录/注册外）需要在请求头中携带 JWT Token
- 请求头格式：`Authorization: Bearer {token}`
- Token 有效期 30 天（可在设置中调整）

#### 数据分区规则
- 每个用户可以有多个应用分区（appId）
- 每个应用分区内可以有多个数据键（key）
- 数据键格式建议：`category.subkey`（如 `settings.theme`）
- 应用分区会自动创建，首次写入数据时自动建立

#### 数据大小限制
- 单用户数据上限：默认 100MB（可在设置中调整）
- 超出限制时返回 413 错误码

#### 同步策略
- **首次同步**：使用 `since=0` 获取所有数据
- **增量同步**：使用上次同步时间戳获取变更
